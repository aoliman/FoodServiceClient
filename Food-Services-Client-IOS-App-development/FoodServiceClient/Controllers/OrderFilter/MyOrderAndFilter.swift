...
class MyOrderAndFilter: UIViewController {
    ...
    func Redrawfilter(){
        ...
        self.setupBtnFilter(title: "Denied", tag: 2, btn: self.customView.Pending)
...
extension MyOrderAndFilter :UITableViewDelegate ,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let orderstatus = orderItems[indexPath.row].status {
            ...
            cell.OrderStatus.text =  orderstatus
            switch orderstatus {
            case "PENDING":
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if (orderItems[indexPath.row].kind == "delivery-guy-order" && orderItems[indexPath.row].status == OrderStatus.ontheWay.rawValue) {
