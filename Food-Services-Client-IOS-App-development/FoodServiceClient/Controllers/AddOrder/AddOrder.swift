    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if (indexPath.row <  Myarryofdata.count) {
          let cell : QuantityCell = tableAddoeder.dequeueReusableCell(withIdentifier: "QuantityCell", for: indexPath) as! QuantityCell
            ...
            cell.Pricelable.text="\("Riyal".localize()) \(String(MyCardOrders[selectcard].count*orderdata.price))"
            EditTotalprice()
            return cell
        }
...
extension AddOrder :Mydatedelgate {
    ...
    var Card=retetrivecard()
            ...
            Alamofire.request("http://165.227.96.25/api/v1/\(Type)/\(Card.ownedid!)/orders", method: .post, parameters: parameters, encoding: JSONEncoding.default, headers: headers).responseJSON { response in
                ...
                case .success:
                    ...
                    if(response.response?.statusCode == 201){
                       ...
                       myLoader.hideCustomLoader()
                        ...
                        UIApplication.shared.keyWindow?.rootViewController?.view.makeToast("Order has been sending")
