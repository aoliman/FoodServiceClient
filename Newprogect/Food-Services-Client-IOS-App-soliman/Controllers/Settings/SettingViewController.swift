...
class SettingViewController: UIViewController{
    ...
    let sideMenuVC = SideMenuViewController()
    lazy var Getallproducterepo = GetallProdacteRepo()
    lazy var settingRepo = SettingRepo()
    
    var collectionView: UICollectionView = UICollectionView(frame: CGRect(),collectionViewLayout: UICollectionViewFlowLayout())
    var sections = ["GeneralSetting"]
    
    //    var userData = UserDefaults.standard.object(forKey: defaultsKey.userData.rawValue) as? [String: Any]
    
    //    var profileSettingIcons = ["fa-user-circle", "fa-envelope", "fa-phone", "fa-lock"]
    //    var profileSetting = ["Name", "Email", "Mobile", "changePassword"]
    //    var profileValues: [String?] = []
    ...
    override func viewDidLoad() {
        ...
        //        profileValues = [user?.name, user?.email, user?.phone, "Password"]
        //
        currentLanguage = UserDefaults.standard.string(forKey: defaultsKey.language.rawValue)
        UserDefaults.standard.synchronize()
...
extension SettingViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        ...
        if indexPath.row == 0 {
            if Localize.currentLanguage() == "ar" {
                cell?.generalSetting.text = "عربي"
            } else {
                cell?.generalSetting.text = "English"
            }
            cell?.icon.text = String.fontAwesomeIcon(code: generalSettingIcons[indexPath.row])
            cell?.label.text = generalSetting[indexPath.row].localized()
        } else if indexPath.row == generalSetting.count - 1  {
            ...
            cell?.icon.text = String.fontAwesomeIcon(code: generalSettingIcons[indexPath.row])
            cell?.visaIcon.isHidden = false
            cell?.masterCardIcon.isHidden = false
            cell?.americanCardIcon.isHidden = false
        } else {
            cell?.icon.text = String.fontAwesomeIcon(code: generalSettingIcons[indexPath.row])
            cell?.label.text = generalSetting[indexPath.row].localized()
            cell?.switchButton.isHidden = false
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if indexPath.row == 0 {
            ...
            let popup = PopupDialog.init(viewController: ChangeLanguageVC()) {
                self.currentLanguage = UserDefaults.standard.string(forKey: defaultsKey.language.rawValue)
    ...
    func setupViewAppearance() {
        ...
        let appToolbarController = AppToolbarController(rootViewController: self)
...
//        let backButton = UIBarButtonItem(title: "Back".localized(), style: .plain, target: self, action: nil)
//        backButton.setTitleTextAttributes([NSAttributedStringKey.font: UIFont.appFontRegular(ofSize: 18)], for: .normal)
...
extension SettingViewController  {
    ...
    func updateLanguage() {
        ...
        self.Getallproducterepo.SendLanguage(Language: Localize.currentLanguage(), completionSuccess: { (resulte) in
            
                        })
        
        
//        settingRepo.changeLanguage(language: currentLanguage!) { (user) in
//            print("userSetting \(user)")
//            UserDefaults.standard.set(user.language, forKey: defaultsKey.language.rawValue)
//
//        }
    }
    
    //    func editPtofile(_ sender: UIButton) {
    //
    //        guard let cell = sender.superview as? SettingCollectionViewCell else {
    //            return // or fatalError() or whatever
    //        }
    ...
    //        if !(cell.value.text?.isEmpty)! {
    //
    //            settingRepo.editProfile(name: (self.user?.name)!, email: (self.user?.email)!, phone: (self.user?.phone)!, currentPassword: self.currentPassword, newPassword: self.newPassword) { [weak self] (user, statusCode) in
    //
    //
    //                switch statusCode {
    //
...
protocol ReloadSideMenuDelegate: class {
    ...
    func reloadTableView(_ user: Bool) // change user information in side meun after editing succesfull
