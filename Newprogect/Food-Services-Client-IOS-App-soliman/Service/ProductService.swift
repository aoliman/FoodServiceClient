...
enum ProductService{
   case SignUp(name :String,email :String,password :String,phone :String,address :String)
    ...
    case creditCardInfo(id: Int, token: String)
    case AddPayment(paymenttoken:String )
    case GetMyPayment()
    case SetdefultePayment(paymentid:String)
    case DeletePayment(paymentid:String)
...
extension ProductService :TargetType  {
   ...
   var baseURL: URL {
       return URL(string: "http://67.205.139.227/api/v1")!
    }
    ...
    var path: String {
        ...
        case .creditCardInfo(let id, _):
            return "/clients/\(id)/credit-card"
        case .AddPayment:
            return "/users/\((Singeleton.userInfo?.id)!)/credit-cards"
        case .GetMyPayment():
            return "/users/\((Singeleton.userInfo?.id)!)/credit-cards"
            
        case .SetdefultePayment(let paymentid) :
                return "/users/\((Singeleton.userInfo?.id)!)/credit-cards/\(paymentid)/default"
        case .DeletePayment(let paymentid) :
            return "/users/\((Singeleton.userInfo?.id)!)/credit-cards/\(paymentid)"
    ...
    var method: Moya.Method {
        switch self {
        case .SignUp , .CreateOrder , .CreateOrderPartyCooker ,.SendOrderDeliveryGuy, .AddPayment:
            return .post
        case .GetProducte,.GetCatTegory,.GetMyPayment():
            return .get
    case .GetHomeCookerPlace ,.GetSpecificHomeCookerProduct ,.GetProfileInfo , .GetSpecificProduct ,.GetPartyCookerCatogary , .GetAllFoodCar , .GetCreditCardInfo ,.GetFoodCarForMap ,.GetDeliveryGuy ,.GetDeliveryGuyInfo ,.GetOrderPriceDeliveryGuy:
            return .get
        case .SendLanguage , .SendOrderRate ,.SendOrderStatus , .creditCardInfo ,.SetdefultePayment:
            return .put
        case .DeletePayment :
            return .delete
        }
   ...
   public   var task: Task {
    ...
    case  .GetHomeCookerPlace , .GetProfileInfo , .GetSpecificProduct , .GetCreditCardInfo ,.GetDeliveryGuyInfo ,.GetMyPayment() , . SetdefultePayment ,.DeletePayment:
        return .requestPlain
    case .GetProducte  ,.GetPartyCookerCatogary ,.GetAllFoodCar ,. GetFoodCarForMap , .GetDeliveryGuy,.GetCatTegory , .creditCardInfo :
        return .requestParameters(parameters: self.parameters!, encoding: self.parameterEncoding)
    case .CreateOrder , .CreateOrderPartyCooker ,.GetOrderPriceDeliveryGuy ,.SendOrderDeliveryGuy , .AddPayment :
      return.requestParameters(parameters: self.parameters!, encoding: self.parameterEncoding)
    ...
    public  var headers: [String : String]? {
       ...
       return ["Content-type": "application/json", "Accept-Language": Localize.currentLanguage()]
        case .GetProducte , .GetHomeCookerPlace , .GetSpecificHomeCookerProduct , .GetProfileInfo , .GetSpecificProduct , .GetPartyCookerCatogary , .CreateOrderPartyCooker ,.GetAllFoodCar , .GetCreditCardInfo , .GetFoodCarForMap , .GetDeliveryGuy , .GetDeliveryGuyInfo,.GetOrderPriceDeliveryGuy ,.SendOrderDeliveryGuy:
            return   ["Content-Type": "application/json" , "Authorization" : "Bearer \(getUserAuthKey())", "Accept-Language": Localize.currentLanguage()]
            case .CreateOrder , .SendLanguage ,.SendOrderRate ,.SendOrderStatus ,.GetCatTegory,.creditCardInfo , .AddPayment ,.GetMyPayment , . SetdefultePayment ,.DeletePayment :
                return  ["Content-Type": "application/json" , "Authorization" : "Bearer \(getUserAuthKey())", "Accept-Language": Localize.currentLanguage() ]
    ...
    public  var parameters: [String: Any]? {
       ...
       case .SignUp(let name,let email,let password,let phone,let address):
        ...
        case .GetProfileInfo , .GetSpecificProduct , .GetCreditCardInfo ,.GetDeliveryGuyInfo ,.GetMyPayment , . SetdefultePayment ,.DeletePayment:
            return nil
        case .CreateOrder(let Card, let type):
        ...
        case .creditCardInfo(_, let token):
            return ["cardToken": token]
            
        case .AddPayment(let paymenttoken):
            return ["newCardToken": paymenttoken]
        }
    ...
    public var parameterEncoding: Moya.ParameterEncoding {
        switch self {
        case .SignUp , .CreateOrder , .CreateOrderPartyCooker ,.SendLanguage ,.SendOrderRate ,.SendOrderStatus  , .SendOrderDeliveryGuy , .creditCardInfo ,.AddPayment  :
            return   JSONEncoding.default
        case .GetProducte , .GetHomeCookerPlace , .GetSpecificHomeCookerProduct , .GetProfileInfo ,.GetSpecificProduct ,.GetPartyCookerCatogary , .GetAllFoodCar , .GetCreditCardInfo , .GetFoodCarForMap  ,.GetDeliveryGuyInfo, .GetDeliveryGuy , .GetOrderPriceDeliveryGuy ,.GetCatTegory ,.GetMyPayment , . SetdefultePayment ,.DeletePayment:
