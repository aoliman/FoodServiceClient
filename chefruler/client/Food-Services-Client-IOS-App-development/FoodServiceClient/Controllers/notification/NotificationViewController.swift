        ...
        if let id = notifications?.data[indexPath.row].subject.id {
                ...
                // have id and get order
                        ...
                        orderrepo.getSingleOrder(orderId:id , stauts: "", userId: (Singeleton.userInfo?.id)!, onSuccess: { (response)  in
                            ...
                            if response.kind == "delivery-guy-order" {
                                if (  (response.status == DeliveryGuyOrderStatus.onTheWay.rawValue || response.status == DeliveryGuyOrderStatus.arrived.rawValue || response.status == DeliveryGuyOrderStatus.delivered.rawValue )) {
                                ...
                                } else if  response.status == DeliveryGuyOrderStatus.refused.rawValue || response.status == DeliveryGuyOrderStatus.pending.rawValue ||
                                    response.status == DeliveryGuyOrderStatus.accepted.rawValue ||
                                    response.status == DeliveryGuyOrderStatus.taken.rawValue{
                                    ...
                                    orderDetailsVC.orderItem = response
...
extension NotificationViewController {
    ...
    func getNotification(_ limit: Int) {
        ...
        notificationRepo.getNotification(page: self.page, limit: 10) { (notification, statusCode) in
            myLoader.hideCustomLoader()
            switch statusCode {
            case StatusCode.complete.rawValue :
