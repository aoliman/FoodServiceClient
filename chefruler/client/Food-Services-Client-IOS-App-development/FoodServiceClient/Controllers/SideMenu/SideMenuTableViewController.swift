...
class SideMenuViewController: UITableViewController, ReloadSideMenuDelegate {
    ...
    override func viewWillLayoutSubviews() {
        ...
        if #available(iOS 11.0, *) {
            ...
            self.view.layoutEdgeInsets = insets
            self.tableView.insetsContentViewsToSafeArea = true
            print(self.tableView.layoutEdgeInsets)
        } else {
            // Fallback on earlier versions
        }
...
//        // safe area values are now available to use
        if #available(iOS 11, *) {
            let window = UIApplication.shared.windows[0]
            let insets:UIEdgeInsets = window.safeAreaInsets
            self.tableView.layoutEdgeInsets = insets
            print(self.tableView.layoutEdgeInsets)
        }
        else
        {
...
//        if let sideViewWidth = self.view.bounds.size.width {
//            newSafeArea.right += sideViewWidth
//        }
//        // Adjust the safe area to accommodate
//        //  the height of the bottom view.
//        if let bottomViewHeight = v.bounds.size.height {
...
//        self.title = "Back".localized()
        ...
        if #available(iOS 11, *) {
            ...
            let insets:UIEdgeInsets = window.safeAreaInsets
            self.view.layoutEdgeInsets = insets
            print(self.view.layoutEdgeInsets)
        }
        else
        {
            // return UIEdgeInsets(top: 0.0, left: 0.0, bottom: 0.0, right: 0.0)
